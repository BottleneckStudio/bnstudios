...
describe("middleware/cors", () => {
  ...
  it("should force credentials false when origin is wildcard", () => {
    ...
    expect(cors).toHaveBeenCalledWith(expect.objectContaining({ origin: "*", credentials: false }));
  });
  ...
  it("should accept custom allowedHeaders", () => {
    ...
    expect(cors).toHaveBeenCalledWith(expect.objectContaining({ allowedHeaders: ["X-Custom"] }));
  });
});
