...
describe('config', () => {
  ...
  describe('validateConfig', () => {
    ...
    it('should throw for invalid port', () => {
      expect(() => validateConfig({ serviceName: 'test', port: -1 })).toThrow('port must be a number')
      expect(() => validateConfig({ serviceName: 'test', port: 70000 })).toThrow('port must be a number')
      expect(() => validateConfig({ serviceName: 'test', port: 'abc' })).toThrow('port must be a number')
    })
    ...
    it('should throw for non-string baseUrlPath', () => {
      expect(() => validateConfig({ serviceName: 'test', baseUrlPath: 123 })).toThrow('baseUrlPath must be a string')
    })

    it('should throw for non-array middlewares', () => {
      expect(() => validateConfig({ serviceName: 'test', middlewares: 'bad' })).toThrow('middlewares must be an array')
    })

    it('should throw for non-object loggerOptions', () => {
      expect(() => validateConfig({ serviceName: 'test', loggerOptions: 'bad' })).toThrow('loggerOptions must be an object')
    })
